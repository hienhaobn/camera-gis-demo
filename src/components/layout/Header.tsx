...
export function Header() {
  ...
  const { setStatusFilter, setSelectedCamera, theme, toggleTheme } =
    useMapStore();
  ...
  const [alarms, setAlarms] = useState([
    ...
    },
    {
      id: 3,
      camId: "CAM-110",
      title: "Phát hiện đỗ xe sai quy định",
      location: "Đường Võ Nguyên Giáp",
      time: "10:42",
      priority: "medium",
    },
  ]);
  ...
  return (
    <header className="flex h-16 shrink-0 items-center justify-between px-4 bg-card/85 backdrop-blur-xl border-b border-border/50 z-10 select-none">
      {/* Left side: Sidebar Trigger & Brand Title & Navigation Tabs */}
      <div className="flex items-center gap-6">
        ...
        {/* Navigation Tabs */}
        <nav className="flex items-center gap-1 bg-muted/20 border border-border/30 rounded-lg p-0.5 font-mono text-[11px]">
          <NavLink
            to="/"
            className={({ isActive }) =>
              cn(
                "flex items-center gap-1.5 px-3 py-1.5 rounded-md transition-all font-bold border",
                isActive
          ...
          >
            ...
            <span className="hidden sm:inline">TƯỜNG CAMERA</span>
          </NavLink>
          <NavLink
            to="/ams"
            className={({ isActive }) =>
      ...
      <div className="flex items-center gap-3">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent
            align="end"
            className="w-[320px] bg-card border-border/60 backdrop-blur-xl"
          >
                ...
                >
                  <div className="flex justify-between w-full text-xs">
                    <span className="font-bold text-foreground flex items-center gap-1.5">
                      <AlertOctagon
                        className={`w-3.5 h-3.5 ${alarm.priority === "critical" ? "text-red-500" : "text-amber-500"}`}
                      />
                      {alarm.title}
                    </span>
