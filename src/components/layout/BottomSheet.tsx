...
export function BottomSheet() {
  ...
  return (
    <div className="absolute bottom-4 left-4 right-4 z-20 md:left-6 md:right-6 h-[400px] md:h-[430px] max-h-[90vh] bg-card/90 border border-border/60 rounded-xl shadow-2xl backdrop-blur-2xl flex flex-col overflow-hidden animate-[slide-up_0.3s_ease-out] select-none text-left">
      {/* Header bar */}
      ...
      <div className="flex-1 flex flex-col md:flex-row min-h-0">
        ...
        <div className="w-full md:w-[350px] p-4 flex flex-col border-b md:border-b-0 md:border-r border-border/30 shrink-0 bg-background/30">
          <div className="relative flex-1 w-full bg-black border border-border/60 rounded-md overflow-hidden flex items-center justify-center select-none">
            {/* Visual Background (Fake street vs offline cctv screen) */}
            {streamActive && selectedCamera.status === "online" ? (
              selectedCamera.streamUrl ? (
                <VideoPlayer src={selectedCamera.streamUrl} name={selectedCamera.name} />
              ) : (
                <img
                  src="/cctv_active.png"
            ...
            {streamActive && selectedCamera.status === "online" ? (
              <>
                ...
                <div className="absolute top-2 left-2 flex flex-col gap-0.5 font-mono text-[8px] text-green-400 z-10 bg-black/60 px-1.5 py-0.5 rounded border border-green-500/20">
                  ...
                  <span className="text-[7px] text-green-500/70 border-t border-green-500/10 pt-0.5 mt-0.5">
                ...
                <ScrollArea className="h-full pr-2">
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    ...
                    <Card className="bg-muted/10 border-border/30 p-2.5 rounded-lg">
                      ...
                      <span className="font-bold text-foreground">
                        {selectedCamera.resolution} CONTRAST
                      </span>
                    </Card>
                    ...
                    <div className="col-span-2 grid grid-cols-2 gap-2 mt-1">
                      ...
                      <span className="font-mono text-right text-foreground">
                        {new Date(selectedCamera.lastSeen).toLocaleTimeString(
                          "vi-VN",
                        )}
                      </span>
                    </div>
                    ...
                    <div className="col-span-2 grid grid-cols-2 gap-2">
                      ...
                      <span className="font-mono text-right text-foreground font-bold">
                        SAN-NVR-
                        {selectedCamera.zone.substring(0, 3).toUpperCase()}-01
                      </span>
                    </div>
                  </div>
                </ScrollArea>
                ...
                <div className="flex-1 flex flex-col gap-2 min-h-0">
                  ...
                  <div className="flex gap-2 justify-end">
                    <Button
                      ...
                      onClick={() => {
                        alert(
                          `Ghi chú cho ${selectedCamera.id} đã được cập nhật hệ thống!`,
                        );
                        setNotes("");
                      }}
                    >
